...
export function buildBattleViewModel(
  ...
  const questionCard = match.currentQuestion
    ? {
        ...
        secondsLeft: Math.max(
          0,
          Math.ceil((Date.parse(match.questionDeadlineAt) - now) / 1000),
        ),
        hint: "",
        statusLabel: buildQuestionStatus(match.phase, isCoolingDown, now, match.questionDeadlineAt),
  ...
  return {
    ...
    topBarTimerLabel: nextQuestionCard
      ? `${nextQuestionCard.secondsLeft} 秒`
...
function buildQuestionStatus(
  ...
  if (Math.ceil((Date.parse(deadlineAt) - now) / 1000) <= 2) {
    return "最后抢答";
  }
